...
namespace ZeroKey.Plugin.Unlock
{
    [Export(typeof(IZeroKeyServicePlugin))]
    public class UnlockWorkstation : IZeroKeyServicePlugin
    {
        ...
        //            Clients.Add(tc);
        //            ZeroKey.Service.Core.ServiceCore.Log("Unlock Workstation: Client connected");
        //        }
        //        catch(Exception ex)
        //        {
        ...
        public void NcfRingDown(string id, Dictionary<string, object> parameters, SystemState state)
        {
            ...
            ZeroKey.Service.Core.ServiceCore.Log("Unlock Workstation: Send data to client");
            try
            {
                TcpClient tc = state.CredentialData.Client;
                ServiceCommunication.SendNetworkMessage(ref tc, (string)parameters["Username"]);
                ServiceCommunication.SendNetworkMessage(ref tc, ZeroKey.Service.Common.Crypto.Decrypt((string)parameters["Password"], id));
            ...
            ProcessAsUser.Launch($"{appDir}\\medatixx\\ZeroKey.Plugin.medatixx.exe -c {id} -s -l");
        }
        ...
        private class ProcessAsUser
        {
            ...
            private static bool LaunchProcessAsUser(string cmdLine, IntPtr token, IntPtr envBlock)
            {
                ...
                if (result == false)
                {
                    ...
                    ZeroKey.Service.Core.ServiceCore.Log("UnlockWorkstationPlugin: Error " + message);
            ...
            public static bool Launch(string appCmdLine /*,int processId*/)
            {
                ...
                if (processId > 1)
                {
                    ...
                    if (!ret)
                    {
                        ZeroKey.Service.Core.ServiceCore.Log("UnlockWorkstationPlugin: unlock failed");
                    }
                ...
                {
                    ZeroKey.Service.Core.ServiceCore.Log("UnlockWorkstationPlugin: process not found");
                }
                return ret;
            }
            private static IntPtr GetPrimaryToken(int processId)
            {
                ...
                catch (ArgumentException)
                {
                    ...
                    ZeroKey.Service.Core.ServiceCore.Log("UnlockWorkstationPlugin: " + details);
                ...
                if (retVal == true)
                {
                    ...
                    ZeroKey.Service.Core.ServiceCore.Log("UnlockWorkstationPlugin: " + message);
                    //Debug.WriteLine(message);
